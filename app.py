        ...
        with tab_harga:
            ...
            if all(c in df.columns for c in ["Open", "High", "Low", "Price"]):
                ...
                fig_candle.update_layout(
                    **chart_layout("Candlestick Chart", "Tanggal", "Harga (IDR)"),
        ...
        with tab_return:
            ...
            st.markdown(
                f"""<div class="interp-box">
                    ...
                    {"(distribusi leptokurtik menunjukkan ekor yang lebih tebal dibandingkan distribusi normal)" if kurt_val > 0 else "(distribusi platikurtik)"},
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if st.session_state.results is None:
        ...
        # ── Tabel Hasil Prediksi Out-of-Sample ─────────────────────────────────
        st.markdown("<div style='height:12px'></div>", unsafe_allow_html=True)
        st.markdown("<div class='subsection-header'>Tabel Hasil Prediksi Data Uji (Out-of-Sample)</div>", unsafe_allow_html=True)
        st.markdown("<div style='height:8px'></div>", unsafe_allow_html=True)

        test_dates_fmt = pd.to_datetime(results["test_dates"])
        selisih = results["y_test"] - results["y_pred"]

        df_pred_outsample = pd.DataFrame({
            "Tanggal": test_dates_fmt.strftime("%d-%m-%Y"),
            "Harga Aktual (IDR)": results["y_test"],
            "Harga Prediksi (IDR)": results["y_pred"],
            "Selisih (IDR)": selisih,
            "Error (%)": results["residual_pct"],
        })

        st.markdown(
            f"""<div class="interp-box">
                Tabel ini mencakup <strong>{len(df_pred_outsample):,} hari perdagangan</strong>
                pada periode data uji, dari <strong>{test_dates_fmt.min().strftime('%d %B %Y')}</strong>
                hingga <strong>{test_dates_fmt.max().strftime('%d %B %Y')}</strong>.
            </div>""",
            unsafe_allow_html=True,
        )
        st.markdown("<div style='height:8px'></div>", unsafe_allow_html=True)
        ...
        urutan = st.radio(
            ...
            horizontal=True,
            key="urutan_tabel_outsample",
        )
        if urutan == "Terbaru ke Terlama":
            df_pred_outsample_display = df_pred_outsample.iloc[::-1].reset_index(drop=True)
        else:
            df_pred_outsample_display = df_pred_outsample

        st.dataframe(
            df_pred_outsample_display.style.format({
                "Harga Aktual (IDR)": "{:,.2f}",
                "Harga Prediksi (IDR)": "{:,.2f}",
                "Selisih (IDR)": "{:,.2f}",
                "Error (%)": "{:.4f}",
            }),
        ...
        csv_outsample = df_pred_outsample.to_csv(index=False).encode("utf-8")
        st.download_button(
            label="Unduh Tabel Prediksi (CSV)",
            data=csv_outsample,
            file_name="hasil_prediksi_outsample.csv",
            mime="text/csv",
        )
        ...
        if calib_diag is not None and not calib_diag.get("skipped", False):
            ...
            st.markdown(
                f"""<div class="note-box">
                    ...
                    diestimasi dari periode kalibrasi (irisan validation split pada data training yang
